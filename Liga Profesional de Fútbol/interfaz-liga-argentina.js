...
function placeTeams(){
    ...
    for (i = 0; i < finalStandings.length; i++){
        ...
        if (i === 0){
            ...
            if(finalStandings[i][0] !== 'Independiente'){
                ...
                const copaLibertadores = document.createElement('div')
                copaLibertadores.className = 'textInfo'
                copaLibertadores.textContent = 'Copa Libertadores: ' + finalStandings[i-5][0] + ', ' + finalStandings[i-4][0] + ', '+ finalStandings[i-3][0] + ', ' + finalStandings[i-2][0] + ', ' + finalStandings[i-1][0] + ' e '+ finalStandings[i][0]+'.'
                info.appendChild(copaLibertadores)
            }
        }
        else if (i === 6){
